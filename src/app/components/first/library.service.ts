...
export class LibraryService extends BaseService {
  ...
  getLibraries(str: string = ''): void {
    ...
    // TODO:sure. we haven't handling errors. This is test task
    // TODO: also I naven't any interfaces for different entities
    forkJoin([this.getData(this.datasetsWithRows, params), this.getData(this.datasets)])
      .pipe(
        ...
        tap(([val, category]) => {
          ...
          const mappedLibraries: Array<ILibrary> = (<Array<any>>val).map(({ Cells }) => {
            const mappedAddress: string = Cells.ObjectAddress.reduce((acc: any, address: { Address: string }) => {
              acc.push(address.Address);
              return acc;
            }, []).join('; ')

            captions.forEach((caption: { Name: string, Caption: string }) => {
              if (typeof Cells[caption.Name] === 'string') {
                // for simple example of library details we get only string parameters
                libraryDetails[`${caption.Name}`] = Cells[`${caption.Name}`];
              }
            });
            return {
          ...
          this.datasets$.next(mappedLibraries);
